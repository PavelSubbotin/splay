...
private:
	...
	int sum(SplayTree *t) {
		return t == NULL ? 0 : t->sum_;
	}

	int sizeLeft(SplayTree *t) {
		return t == NULL ? 0 : t->sizeLeft_;
	}

	int sizeRight(SplayTree *t) {
		return t == NULL ? 0 : t->sizeRight_;
	}
	...
	void sets(SplayTree *t, int push) {
		if (t == NULL) {
			return;
		}

		t->sum_ += push * getSize(t);
		t->key_ += push;
		t->leftist_ += push;
		t->rightist_ += push;
		t->push_ += push;
	...
	void push(SplayTree *t) {
		...
		if (t->push_ != 0) {
			sets(t->left_, t->push_);
			sets(t->right_, t->push_);
			t->push_ = 0;
		}
...
public:
	...
	SplayTree* changeSegment(SplayTree *t, int left, int right, int element) {
		SplayTree *l = NULL, *m = NULL, *r = NULL;
		split(t, right + 1, l, r);
		split(l, left, l, m);
